...
func TestWatchRestore(t *testing.T) {
	test := func(delay time.Duration) func(t *testing.T) {
		return func(t *testing.T) {
			b, tmpPath := backend.NewDefaultTmpBackend()
			s := newWatchableStore(zap.NewExample(), b, &lease.FakeLessor{}, metacache.NewConsistentIndex(b.BatchTx()), StoreConfig{})
			...
			newStore := newWatchableStore(zap.NewExample(), newBackend, &lease.FakeLessor{}, metacache.NewConsistentIndex(newBackend.BatchTx()), StoreConfig{})
...
func TestWatchRestoreSyncedWatcher(t *testing.T) {
	...
	s1 := newWatchableStore(zap.NewExample(), b1, &lease.FakeLessor{}, metacache.NewConsistentIndex(b1.BatchTx()), StoreConfig{})
	defer cleanup(s1, b1, b1Path)

	b2, b2Path := backend.NewDefaultTmpBackend()
	s2 := newWatchableStore(zap.NewExample(), b2, &lease.FakeLessor{}, metacache.NewConsistentIndex(b2.BatchTx()), StoreConfig{})
