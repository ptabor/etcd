...
err := endpoints.ManagerAddEndpoint(context.TODO(), em, "foo/bar/my-service/e1", endpoints.Endpoint{Addr:"1.2.3.4"});
```
...
```go
em := endpoints.NewManager(client, "foo/bar/my-service")
err := endpoints.ManagerDeleteEndpoint(context.TODO(), em, "foo/bar/my-service/e1");
```
...
If its desired to modify multiple endpoints in a single transaction, `endpoints.Manager` can be used directly:
